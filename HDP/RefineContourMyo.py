...
sys.path.append(R'E:\Work\HDP-Working\HDP')
...
def plotCtrOnImage(fname, img, edgeIm, nCtrs, oCtrs, loc, imName, DICE=None, radius=None) :
    ...
    fParts = ['E:', 'Work', 'HDP-Working', 'HDP', 'KMC', '440991', 'Result', 'Ctr_image_03.dcm']
...
def mainForDP(image_data,contour,ctrCentre, userPoint) :
    global fp, data
    DPObj = DynProg()
    ctrsRef = [None, None]
    ...
    for i in range(1):  # for both endo and epicardium contours
        ...
        ctrsRef[i] = transformCtrBack(ctrCorrected, loc, 120, [first_angle,last_angle])

    refinedContour = ctrsRef[0]
...
def mainFunc(contour,ctrCentre,userPoint,image_data,standalone=False):
    ONECARDIO = False
    # root = Path(r'.')
    root = Path(r'E:\Work\HDP-Working\HDP')
    for hosp in ['KMC']: #os.listdir(root):
        if (root / hosp).is_dir() is False: continue
        for pat in os.listdir(root/hosp):
            refinedContour = mainForDP(image_data,contour,ctrCentre, userPoint)
    return refinedContour
