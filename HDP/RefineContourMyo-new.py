...
def mainForDP(patient, GT, type='HHT_LGE') :
    global fp, data
    DPObj = DynProg()
    ctrsRef = [None, None]
    ...
    for folder in folders :
        for SOPId in GT :
            ...
            # SF.showImage(img, imName + ' - Original image', show=True, block=True)
            ...
            # print("-----------------------")
            # print(ctrs[0]) #endocardium
            # print(ctrs[1]) #epicardium
            # print("-----------------------")
            ...
            tCtr = transformCtr(ctr, loc, 120)  # Cartesian coordinate system to Polar coordinate system
            ...
            # t_ctr = transformCtrBack(tCtrs, loc)
            # t_ctr = t_ctr[:-2]
...
def mainFunc():
    ...
    for hosp in ['KMC']: #os.listdir(root):
        ...
        for pat in os.listdir(root/hosp):
            data['Pat Folder'] = str(root/hosp/pat)
            GT = readGroundTruth(root/hosp/pat)
            refinedContour = mainForDP(root/hosp/pat, GT)
    fp.close()
    return refinedContour
